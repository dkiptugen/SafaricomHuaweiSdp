...
require_once(APPPATH.'third_party/lib/client_inc.php');
/*
...
class SDPService{
	...
	function sendSms($kmp_spid,$kmp_sppwd,$kmp_service_id,$kmp_timestamp,$kmp_recipients,$kmp_correlator,$kmp_code,$kmp_message)
		{
			...
			$kmp_send_sms_notify_service_endpoint 	= $this->con->kmp_send_sms_notify_service_endpoint;
			$kmp_send_sms_service_endpoint 			= $this->con->kmp_send_sms_service_endpoint;
			$kmp_send_sms_service_log_file_prefix	= $this->con->kmp_send_sms_service_log_file_prefix;
			$kmp_send_sms_service_max_recipient		= $this->con->kmp_send_sms_service_max_recipient;
			$kmp_send_sms_service_log_soap_messages	= $this->con->kmp_send_sms_service_log_soap_messages;
			$kmp_send_sms_service_log_file_prefix	= $this->con->kmp_send_sms_service_log_file_prefix;
			$kmp_send_sms_service_debug				= $this->con->kmp_send_sms_service_debug;
			$kmp_send_sms_notify_delivery           = $this->con->kmp_send_sms_notify_delivery;
			...
			if(is_array($kmp_recipients))
				{
					...
					if($count > $this->con->kmp_send_sms_service_max_recipient)
						{ 
							//too many recipients
			...
			$client = new nusoap_client($this->con->kmp_send_sms_service_endpoint,true);	
			$bsoapaction = "";
			$client->soap_defencoding = 'utf-8';
			$client->useHTTPPersistentConnection();
			...
			if($kmp_send_sms_service_log_soap_messages == 1) 
				{ 
					...
					//check whether to log the client debug str 
					if($this->con->kmp_send_sms_service_debug == 1){ 
						$debug_str=$client->debug_str;
					}
					$data = $timestamp."|".$operation."|".$request."|".$response."|".$debug_str."\n";
					$this->writeToFile($service_log_file,$data);
	...
	*/
	function getSmsDeliveryStatus($kmp_spid,$kmp_sppwd,$kmp_service_id,$kmp_timestamp,$kmp_request_identifier,$kmp_msisdn='')
		{		
			$kmp_get_sms_delivery_status_service_endpoint 			= $this->con->kmp_get_sms_delivery_status_service_endpoint;
			$kmp_get_sms_delivery_status_service_log_soap_messages 	= $this->con->kmp_get_sms_delivery_status_service_log_soap_messages;
			$kmp_get_sms_delivery_status_service_log_file_prefix 	= $this->con->kmp_get_sms_delivery_status_service_log_file_prefix;
			$kmp_get_sms_delivery_status_service_debug 				= $this->con->kmp_get_sms_delivery_status_service_debug;
